...
interface PayrollContextType {
  ...
  activeYear: number;
  customLeaveDeductions: Record<string, number>;
  setCustomLeaveDeduction: (employeeId: string, year: number, month: number, amount: number | null) => void;
  customAdjustedHours: Record<string, number>;
  setCustomAdjustedHours: (employeeId: string, year: number, month: number, hours: number | null) => void;
  bulkSetCustomAdjustedHours: (updates: Record<string, number | null>, year: number, month: number) => void;
  ...
  clearMonthAttendance: (year: number, monthIndex: number) => void;
...
const initialEmployees: Employee[] = [
  ...
  { id: 'EMP007', name: 'Aanchal Goel', type: 'Staff', basicSalary: 48000, standardShiftStart: '08:00', esiDeducted: true, pfDeducted: true, lwfDeducted: true },
];
...
export const PayrollProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [customLeaveDeductions, setCustomLeaveDeductions] = useState<Record<string, number>>(() => {
    const saved = localStorage.getItem('payroll_mgmt_custom_leave_deductions');
    if (saved) {
      try {
        return JSON.parse(saved);
      } catch (e) {
        console.error('Error parsing custom leave deductions from localStorage', e);
      }
    }
    return {};
  });

  const [customAdjustedHours, setCustomAdjustedHoursState] = useState<Record<string, number>>(() => {
    const saved = localStorage.getItem('payroll_mgmt_custom_adjusted_hours');
    if (saved) {
      try {
        return JSON.parse(saved);
      } catch (e) {
        console.error('Error parsing custom adjusted hours from localStorage', e);
      }
    }
    return {};
  });
  ...
  const [advances, setAdvances] = useState<Record<string, number>>(() => {
    const saved = localStorage.getItem('payroll_mgmt_advances');
    if (saved) {
      try {
        return JSON.parse(saved);
      } catch (e) {
        console.error('Error parsing advances from localStorage', e);
      }
    }
    return {};
  });
  ...
  useEffect(() => {
    localStorage.setItem('payroll_mgmt_custom_leave_deductions', JSON.stringify(customLeaveDeductions));
  }, [customLeaveDeductions]);

  useEffect(() => {
    localStorage.setItem('payroll_mgmt_custom_adjusted_hours', JSON.stringify(customAdjustedHours));
  }, [customAdjustedHours]);

  useEffect(() => {
    localStorage.setItem('payroll_mgmt_advances', JSON.stringify(advances));
  }, [advances]);

  const setCustomLeaveDeduction = (employeeId: string, year: number, month: number, amount: number | null) => {
    const key = `${employeeId}-${year}-${month}`;
    setCustomLeaveDeductions((prev) => {
      const copy = { ...prev };
  ...
  const setCustomAdjustedHours = (employeeId: string, year: number, month: number, hours: number | null) => {
    const key = `${employeeId}-${year}-${month}`;
    setCustomAdjustedHoursState((prev) => {
      const copy = { ...prev };
  ...
  const bulkSetCustomAdjustedHours = (updates: Record<string, number | null>, year: number, month: number) => {
    setCustomAdjustedHoursState((prev) => {
      const copy = { ...prev };
      Object.entries(updates).forEach(([empId, hours]) => {
  ...
  const setAdvance = (employeeId: string, year: number, month: number, amount: number | null) => {
    const key = `${employeeId}-${year}-${month}`;
    setAdvances((prev) => {
      const copy = { ...prev };
  ...
  const bulkSetAdvances = (updates: Record<string, number | null>, year: number, month: number) => {
    setAdvances((prev) => {
      const copy = { ...prev };
      Object.entries(updates).forEach(([empId, amount]) => {
        const key = `${empId}-${year}-${month}`;
        if (amount === null || isNaN(amount) || amount === 0) {
          delete copy[key];
        } else {
  ...
  const clearMonthAttendance = (year: number, monthIndex: number) => {
    const monthPrefix = `${year}-${String(monthIndex + 1).padStart(2, '0')}-`;
    setAttendanceRecords((prev) => prev.filter((r) => !r.date.startsWith(monthPrefix)));
  };
  ...
  const resetToDemoData = () => {
    ...
    localStorage.removeItem('payroll_mgmt_custom_leave_deductions');
    localStorage.removeItem('payroll_mgmt_custom_adjusted_hours');
    localStorage.removeItem('payroll_mgmt_advances');
    setEmployees(initialEmployees);
    setAttendanceRecords(generateMockAttendance(initialEmployees));
    setDeletedLogs([]);
    setCustomLeaveDeductions({});
    setCustomAdjustedHoursState({});
    setAdvances({});

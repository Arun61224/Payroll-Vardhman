...
import { Calendar, Save, Check, Clock, AlertTriangle, Moon, HelpCircle, CheckCircle2, UserCheck, RefreshCw, Upload, X, FileSpreadsheet, Download, Trash2 } from 'lucide-react';
...
export const AttendanceTracker: React.FC = () => {
  const { employees, attendanceRecords, activeDate, saveAttendance, setActiveDate, clearMonthAttendance } = usePayroll();
  ...
  // Clear Month Attendance Modal States
  const [isClearMonthModalOpen, setIsClearMonthModalOpen] = useState(false);
  const [clearYear, setClearYear] = useState<number>(() => parseInt(activeDate.split('-')[0], 10) || 2026);
  const [clearMonth, setClearMonth] = useState<number>(() => {
    const parsed = parseInt(activeDate.split('-')[1], 10);
    return isNaN(parsed) ? 5 : parsed - 1;
  });
  ...
  const handleBulkPreset = (presetStatus: AttendanceStatus) => {
    setLocalRecords((prev) => {
      const updated = { ...prev };
      employees.forEach((emp) => {
        const isDiscontinued = emp.discontinuedDate && activeDate > emp.discontinuedDate;
        if (isDiscontinued) {
          // Keep as clean/absent/unmarked
          return;
        }
      ...
      <AnimatePresence>
        ...
        {clearSuccessMsg && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: -20 }}
            className="fixed top-4 right-4 z-50 bg-rose-900 text-white px-4 py-3 rounded-xl shadow-lg flex items-center gap-2 border border-rose-800"
          >
            <Trash2 className="h-4 w-4 text-rose-300" />
            <span className="text-sm font-medium">{clearSuccessMsg}</span>
          </motion.div>
        )}
      </AnimatePresence>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <button
            onClick={() => {
              ...
              if (parts.length === 3) {
                setClearYear(parseInt(parts[0], 10) || 2026);
                const m = parseInt(parts[1], 10) - 1;
                setClearMonth(isNaN(m) ? 5 : m);
              }
              setIsClearMonthModalOpen(true);
            }}
            className="px-3.5 py-2 text-xs font-semibold text-rose-700 bg-rose-50 hover:bg-rose-100 transition rounded-xl flex items-center gap-1.5 border border-rose-100 cursor-pointer"
            title="Wipe or clear attendance logs for a specific month (e.g. June)"
          >
            <Trash2 className="h-4 w-4 text-rose-600" />
            <span>Clear Month Logs</span>
          </button>
              ...
              return (
                <div key={emp.id} className="p-5 flex flex-col lg:flex-row lg:items-center gap-5 hover:bg-slate-50/20 transition-colors">
                  ...
                  <div className="lg:w-1/4 space-y-1">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {emp.discontinuedDate ? (
                        <span className="text-[10px] text-rose-600 font-semibold">Discontinued since {emp.discontinuedDate}</span>
                      ) : (
                  ...
                  <div className="lg:w-1/4">
                    {isDiscontinued ? (
                      <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold bg-rose-50 border border-rose-100 text-rose-700">
                    ...
                    {isDiscontinued ? (
                      <div className="flex-1 py-2 text-center border-2 border-dashed border-slate-100 rounded-xl bg-slate-50/50 flex items-center justify-center">
                        <span className="text-xs text-slate-400 font-medium italic">
                          Employment discontinued. No attendance required.
                        </span>
                      </div>
                    ) : isPresent ? (
                      <>
        ...
        {isClearMonthModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs">
            <motion.div
              ...
              className="bg-white rounded-2xl max-w-md w-full p-6 space-y-5 shadow-2xl border border-slate-100"
            >
              <div className="flex items-center justify-between pb-3 border-b border-slate-100">
                <div className="flex items-center gap-2 text-rose-600">
                  <Trash2 className="h-5 w-5" />
                  <h3 className="font-bold text-slate-800 text-base">Clear Monthly Attendance</h3>
                </div>
                <button
              ...
              <div className="space-y-4 text-xs text-slate-600">
                <p className="leading-relaxed font-medium">
                  Select the month & year whose attendance records you want to completely delete/clear:
                </p>

                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-[11px] font-bold text-slate-700 uppercase tracking-wide mb-1">Select Month</label>
                    <select
                      value={clearMonth}
                  ...
                  <div>
                    <label className="block text-[11px] font-bold text-slate-700 uppercase tracking-wide mb-1">Select Year</label>
                    <select
                      value={clearYear}
                ...
                {(() => {
                  const prefix = `${clearYear}-${String(clearMonth + 1).padStart(2, '0')}-`;
                  const totalInMonth = attendanceRecords.filter((r) => r.date.startsWith(prefix)).length;
                  return (
                    <div className="bg-rose-50/80 border border-rose-200 rounded-xl p-3.5 space-y-1">
                      <div className="font-bold text-rose-950 flex items-center justify-between">
                        <span>Attendance Records Found:</span>
                        <span className="text-xs font-black text-rose-700 px-2 py-0.5 bg-rose-100 rounded-md">{totalInMonth} logs</span>
                      </div>
                      <p className="text-[11px] text-rose-800 leading-normal pt-1">
                        {totalInMonth > 0
                          ? `⚠️ This will permanently erase all ${totalInMonth} attendance punch logs recorded for ${MONTH_NAMES[clearMonth]} ${clearYear}.`
                          : `No attendance records exist for ${MONTH_NAMES[clearMonth]} ${clearYear}.`}
                      </p>
                    </div>
                  );
                })()}
              ...
              <div className="flex items-center justify-end gap-3 pt-2">
                <button
                  onClick={() => setIsClearMonthModalOpen(false)}
                  className="px-4 py-2 border border-slate-200 text-slate-700 hover:bg-slate-50 rounded-xl font-bold text-xs transition cursor-pointer"
                >
                  Cancel
                </button>
                <button
                  disabled={attendanceRecords.filter(r => r.date.startsWith(`${clearYear}-${String(clearMonth + 1).padStart(2, '0')}-`)).length === 0}
                  onClick={() => {
                    const prefix = `${clearYear}-${String(clearMonth + 1).padStart(2, '0')}-`;
                    const totalInMonth = attendanceRecords.filter((r) => r.date.startsWith(prefix)).length;
                    clearMonthAttendance(clearYear, clearMonth);
                    if (activeDate.startsWith(prefix)) {
                      setLocalRecords({});
                    }
                    setIsClearMonthModalOpen(false);
                    setClearSuccessMsg(`Successfully deleted ${totalInMonth} attendance records for ${MONTH_NAMES[clearMonth]} ${clearYear}!`);
                    setTimeout(() => setClearSuccessMsg(null), 4000);
                  }}
                  className="px-4 py-2 bg-rose-600 hover:bg-rose-700 disabled:opacity-40 disabled:cursor-not-allowed text-white rounded-xl font-bold text-xs transition shadow-xs flex items-center gap-1.5 cursor-pointer"
                >
                  <Trash2 className="h-4 w-4" />
                  Delete {MONTH_NAMES[clearMonth]} Attendance
                </button>
              </div>
            </motion.div>
          </div>

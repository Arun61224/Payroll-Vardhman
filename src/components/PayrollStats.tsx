...
export const PayrollStats: React.FC = () => {
  const { employees, attendanceRecords, activeMonth, activeYear, customLeaveDeductions, customAdjustedHours } = usePayroll();

  // Compute payroll summary data for the selected period
  const summaries = useMemo(() => {
    return employees.map((emp) => {
      const overrideKey = `${emp.id}-${activeYear}-${activeMonth}`;
      const leaveOverride = customLeaveDeductions[overrideKey];
      const hoursOverride = customAdjustedHours[overrideKey];
      return calculateMonthlySummary(emp, attendanceRecords, activeYear, activeMonth, leaveOverride, hoursOverride);
    });
  }, [employees, attendanceRecords, activeYear, activeMonth, customLeaveDeductions, customAdjustedHours]);
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-emerald-50/40 p-5 rounded-2xl border border-emerald-100/30 flex items-start gap-4">
          ...
          <div className="space-y-1">
            ...
            <p className="text-xs text-slate-600">Total leaves are safely monitored. Staff leave count is monitored against the <strong className="text-emerald-700">36-hour quarterly allowance</strong> (April–June, July–September), while Labour remains at 0.</p>
          </div>
        </div>

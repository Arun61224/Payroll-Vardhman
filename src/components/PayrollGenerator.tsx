...
export const PayrollGenerator: React.FC = () => {
  ...
  const exportToExcel = () => {
    ...
    const headers = [
      ...
      "Employee Type",
      "Basic Salary (INR)",
      "Derived Hourly Wage (INR)",
      "Days Present",
      "Days Leave",
      "Days Absent",
      "Quarterly Leave Quota Total (Staff)",
      "Quota Hours Used (Staff)",
      "Quota Hours Balance (Staff)",
      "Excess Leave Hours Deducted (Staff)",
      "Custom Adjusted Hours (Staff)",
      "Custom Adjusted Hours Pay (INR)",
      "Late Deductions (INR)",
      "Underwork Deductions (INR)",
      "Excess Leave / Absence Deductions (INR)",
      "Unused Leave Bonus (INR)",
      "Overtime Pay (INR)",
      "Flat Bonus (INR)",
      "Sunday Overtime Pay (INR)",
    ...
    const rows = exportData.map(s => {
      ...
      const quotaRemaining = isStaff ? `${s.juneAugRemainingQuotaHours ?? 0} hrs` : "N/A";
      const excessHours = isStaff ? `${Math.max(0, (s.juneAugMissedHours ?? 0) - 36)} hrs` : "N/A";
      const adjHours = isStaff && s.adjustedHours ? `${s.adjustedHours > 0 ? '+' : ''}${s.adjustedHours} hrs` : "0 hrs";
      const adjHoursPay = isStaff ? Math.round(s.adjustedHoursPay || 0) : 0;
      const sundayPay = Math.round(s.dailyDetails.filter(d => d.isSunday && d.dailyWage > 0).reduce((acc, d) => acc + d.dailyWage, 0));
      ...
      return [
        ...
        Math.round(s.juneAugUnusedLeaveBonus || 0),
        Math.round(s.totalOvertimePay || 0),
        Math.round(s.totalFlatBonuses || 0),
        sundayPay,
  ...
  const exportDetailedToExcel = () => {
    ...
    const isStaff = selectedSummary.employeeType === 'Staff';
    const metadata = [
      ...
      ["Quarterly Leave Quota Total", isStaff ? "36 hours" : "N/A"],
      ["Quota Hours Used / Adjusted", isStaff ? `${selectedSummary.juneAugMissedHours ?? 0} hours` : "N/A"],
      ["Remaining Quota Balance", isStaff ? `${selectedSummary.juneAugRemainingQuotaHours ?? 0} hours` : "N/A"],
      ["Excess Leave Hours (>36h)", isStaff ? `${Math.max(0, (selectedSummary.juneAugMissedHours ?? 0) - 36)} hours` : "N/A"],
      ["Custom Adjusted Hours", isStaff && selectedSummary.adjustedHours ? `${selectedSummary.adjustedHours > 0 ? '+' : ''}${selectedSummary.adjustedHours} hours` : "0 hours"],
      ["Custom Adjusted Hours Pay", `INR ${Math.round(selectedSummary.adjustedHoursPay || 0)}`],
      ["Excess Leave Deduction Amount", `INR ${Math.round(selectedSummary.leaveDeductions)}`],
      ["Unused Leave Bonus Amount", `INR ${Math.round(selectedSummary.juneAugUnusedLeaveBonus || 0)}`],
    ...
    const rows = selectedSummary.dailyDetails.map(day => [
      ...
      Math.round(day.overtimePay || 0),
      Math.round(day.flatBonus || 0),
    ...
    <div className="space-y-6">
      ...
      <div className="bg-indigo-50/50 border border-indigo-100/40 p-4 rounded-2xl grid grid-cols-1 md:grid-cols-3 gap-4 text-xs text-indigo-950/90 font-medium">
        ...
        <div className="space-y-0.5 border-t md:border-t-0 md:border-l border-indigo-100/50 md:pl-4 pt-2 md:pt-0">
          <span className="text-indigo-600 font-bold uppercase tracking-wide text-[10px] block">Leave Quota (April–June, July–September)</span>
          Staff gets 36 hours flexible leave quota per 3-month quarter (unused hours paid as bonus in 3rd month). Labour has 0 paid leaves.
        </div>
        <div className="space-y-0.5 border-t md:border-t-0 md:border-l border-indigo-100/50 md:pl-4 pt-2 md:pt-0">
                    ...
                    <tr key={s.employeeId} className="hover:bg-slate-50/40 transition-colors">
                      ...
                      <td className="py-4 px-6">
                        <div className="space-y-1">
                          ...
                          {s.employeeType === 'Staff' && s.adjustedHours !== undefined && s.adjustedHours !== 0 && (
                            <div className="text-[10px] font-bold text-indigo-700 bg-indigo-50 border border-indigo-100 px-1.5 py-0.5 rounded w-fit">
                              Adjusted: {s.adjustedHours > 0 ? '+' : ''}{s.adjustedHours} hrs ({s.adjustedHoursPay && s.adjustedHoursPay >= 0 ? '+' : ''}₹{s.adjustedHoursPay?.toFixed(0)})
                            </div>
                          )}
                          {s.totalOvertimeBonuses === 0 && !s.dailyDetails.some(d => d.isSunday && d.dailyWage > 0) && (!s.adjustedHours || s.adjustedHours === 0) && (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-2 border border-slate-100 p-4 rounded-2xl bg-white">
                    ...
                    <div className="divide-y divide-slate-100 text-xs">
                      ...
                      <div className="py-2 flex items-center justify-between">
                        <span className="text-slate-600">Overtime Hourly Pay</span>
                        <span className="font-semibold text-emerald-600">+₹{(selectedSummary.totalOvertimePay || 0).toFixed(0)}</span>
                      </div>
                      <div className="py-2 flex items-center justify-between">
                        <span className="text-slate-600">Flat Stay Bonus (₹100)</span>
                        <span className="font-semibold text-emerald-600">+₹{(selectedSummary.totalFlatBonuses || 0).toFixed(0)}</span>
                      </div>
                      <div className="py-2 flex items-center justify-between">
                      ...
                      {selectedSummary.juneAugUnusedLeaveBonus !== undefined && selectedSummary.juneAugUnusedLeaveBonus > 0 && (
                        <div className="py-2 flex items-center justify-between bg-emerald-50/70 px-2 rounded-lg my-1">
                          <span className="text-emerald-900 font-semibold">Unused Leave Bonus ({selectedSummary.quarterName || 'Quarterly'})</span>
                          <span className="font-bold text-emerald-700">+₹{selectedSummary.juneAugUnusedLeaveBonus.toFixed(0)}</span>
                        </div>
                      )}
                      {selectedSummary.employeeType === 'Staff' && selectedSummary.juneAugRemainingQuotaHours !== undefined && (
                        <div className="bg-indigo-50/80 border border-indigo-100 p-3 my-2 rounded-xl text-xs space-y-2">
                          <div className="flex items-center justify-between text-indigo-950 font-bold">
                            <span>{selectedSummary.quarterName || 'Quarterly'} Leave Quota (36 hours):</span>
                            <span className={`px-2 py-0.5 rounded-full text-xs font-bold ${selectedSummary.juneAugRemainingQuotaHours > 0 ? 'bg-indigo-100 text-indigo-700' : 'bg-rose-100 text-rose-700'}`}>
                              {selectedSummary.juneAugRemainingQuotaHours}h remaining
                            </span>
                          </div>

                          {/* Month-by-Month Short Hours Addition Breakdown */}
                          {selectedSummary.quarterBreakdown && selectedSummary.quarterBreakdown.length > 0 && (
                            <div className="p-2 bg-white/80 rounded-lg border border-indigo-100/70 space-y-1">
                              <span className="text-[10px] uppercase tracking-wider font-bold text-indigo-600 block">
                                Quarterly Months Short Hours:
                              </span>
                              <div className="flex items-center gap-1.5 flex-wrap text-xs">
                                {selectedSummary.quarterBreakdown.map((item, idx) => (
                                  <span key={item.monthIndex} className="inline-flex items-center">
                                    {idx > 0 && <span className="mx-1 text-indigo-400 font-bold">+</span>}
                                    <span className="bg-indigo-50 text-indigo-900 border border-indigo-200/60 px-2 py-0.5 rounded-md font-mono font-semibold">
                                      {item.monthName}: {item.missedHours}h
                                    </span>
                                  </span>
                                ))}
                                <span className="text-indigo-400 font-bold mx-0.5">=</span>
                                <span className="bg-indigo-600 text-white px-2 py-0.5 rounded-md font-mono font-bold shadow-xs">
                                  {selectedSummary.juneAugMissedHours || 0}h Total
                                </span>
                              </div>
                            </div>
                          )}
                          ...
                          <p className="text-[11px] text-indigo-900 leading-relaxed">
                            Used <strong>{selectedSummary.juneAugMissedHours || 0}h</strong> of 36h quota across {selectedSummary.quarterName || 'this quarter'}.
                            {selectedSummary.juneAugRemainingQuotaHours > 0
                              ? ` (${selectedSummary.juneAugRemainingQuotaHours}h remaining). Unused hours will be paid out as extra bonus in 3rd month.`
                              : ` Quota reached. Excess ${Math.max(0, (selectedSummary.juneAugMissedHours || 0) - 36)}h are deducted.`}
                          </p>
                        </div>
                      )}
                      {selectedSummary.employeeType === 'Staff' && selectedSummary.adjustedHours !== undefined && selectedSummary.adjustedHours !== 0 && (
                        <div className="py-2.5 px-3 bg-indigo-50/80 border border-indigo-100 rounded-xl my-1.5 flex items-center justify-between">
                          <div>
                            <span className="text-indigo-950 font-bold block">Custom Hours Adjustment</span>
                            <span className="text-[10px] text-indigo-700">Adjusted: {selectedSummary.adjustedHours > 0 ? '+' : ''}{selectedSummary.adjustedHours} hrs @ ₹{selectedSummary.hourlyWage.toFixed(2)}/h</span>
                          </div>
                          <span className={`font-bold ${selectedSummary.adjustedHoursPay && selectedSummary.adjustedHoursPay >= 0 ? 'text-emerald-700' : 'text-rose-700'}`}>
                            {selectedSummary.adjustedHoursPay && selectedSummary.adjustedHoursPay >= 0 ? '+' : ''}₹{selectedSummary.adjustedHoursPay?.toFixed(0)}
                          </span>
                        </div>
                      )}
                    ...
                    <div className="divide-y divide-slate-100 text-xs">
                      ...
                      <div className="py-2 flex items-center justify-between">
                        <span className="text-slate-600">
                          {selectedSummary.employeeType === 'Staff' ? 'Excess Leave Deduction (>36h Quota)' : 'Leaves & Absence Deduction'}
                        </span>
                    ...
                    <table className="w-full text-left border-collapse text-xs">
                      <thead>
                        <tr className="bg-slate-50 sticky top-0 border-b border-slate-100">
                          ...
                          <th className="py-2.5 px-4 font-bold text-slate-500">Overtime</th>
                          <th className="py-2.5 px-4 font-bold text-slate-500">Bonus (₹100)</th>
                        ...
                        {selectedSummary.dailyDetails.map((day) => (
                          <tr key={day.date} className={`hover:bg-slate-50/60 ${day.isSunday ? 'bg-amber-50/20' : ''}`}>
                            ...
                            <td className="py-2.5 px-4 text-purple-700 font-bold font-mono">
                              {day.overtimePay && day.overtimePay > 0 ? `+₹${day.overtimePay.toFixed(0)}` : '—'}
                            </td>
                            <td className="py-2.5 px-4 text-indigo-700 font-bold font-mono">
                              {day.flatBonus && day.flatBonus > 0 ? `+₹${day.flatBonus.toFixed(0)}` : '—'}
                            </td>

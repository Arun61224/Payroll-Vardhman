...
export const EmployeeDashboard: React.FC = () => {
    ...
    try {
      ...
      const rows = lines.slice(1).map((line, idx) => {
        ...
        if (hasIdColumn) {
          ...
          pfStr = parts[6];
          lwfStr = parts[7];
          if (parts[8] && parts[8].trim() && parts[8].trim() !== '-' && !['active', 'na', 'null'].includes(parts[8].trim().toLowerCase())) {
            discontinuedDate = parts[8].trim();
          }
        } else {
          if (parts.length < 3) {
          ...
          pfStr = parts[5];
          lwfStr = parts[6];
          if (parts[7] && parts[7].trim() && parts[7].trim() !== '-' && !['active', 'na', 'null'].includes(parts[7].trim().toLowerCase())) {
            discontinuedDate = parts[7].trim();
          }
        }
  ...
  const exportEmployeesToCsv = (onlySelected = false) => {
    ...
    const rows = targetEmployees.map(emp => {
      ...
      const hourlyWage = Math.round(dailyWage / 9);
      const status = emp.discontinuedDate ? 'Discontinued' : 'Active';
      ...
      return [
        ...
        emp.type,
        emp.basicSalary,
        emp.standardShiftStart || '08:00',
        emp.esiDeducted !== false ? 'Yes' : 'No',
        emp.pfDeducted !== false ? 'Yes' : 'No',
        emp.lwfDeducted !== false ? 'Yes' : 'No',
              ...
              {selectedIds.length > 0 && (
                <>
                  <button
                    ...
                    className="px-4 py-2 text-xs font-bold text-white bg-rose-600 hover:bg-rose-700 transition-all shadow-sm rounded-xl flex items-center gap-1.5 border border-rose-500 cursor-pointer"
                  ...
                  <button
                    onClick={() => exportEmployeesToCsv(true)}
                    className="px-4 py-2 text-xs font-semibold text-emerald-700 bg-emerald-50 hover:bg-emerald-100 transition-colors rounded-xl flex items-center gap-1.5 border border-emerald-200 cursor-pointer"
                    title="Export only selected employees to CSV"
                  >
